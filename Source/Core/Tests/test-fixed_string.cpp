...
#include <..\Include\Core\FixedString.h>
...
{
    utils::transform(std::begin(from), std::end(from), std::begin(to),
...
constexpr T<N - 1> to_fs_2(literal_ref<N> from)
{
    T<N - 1> to;
    utils::transform(std::begin(from), std::end(from), std::begin(to),
                     [](const auto a) { return static_cast<typename T<0>::value_type>(a); });
    return to;
}
} // namespace utils
...
void check()
{
    T str;
    REQUIRE(
        std::all_of(str.begin(), str.end(), utils::equals(static_cast<typename T::value_type>(0))));
}
} // namespace default_construction
...
TEST_CASE("Construction")
{
    ...
    SECTION("Default construction makes zero-filled string")
    {
        ...
        SECTION("fixed_wstring")
        {
            check<wfs>();
        }
        SECTION("fixed_u8string")
        {
            check<u8fs>();
        }
        SECTION("fixed_u16string")
        {
            check<u16fs>();
        }
        SECTION("fixed_u32string")
        {
            check<u32fs>();
        }
    }
