...
void init_huffman(void)
{
    ...
    huff_init(&msg_huffman);
    
    for (int i = 0; i < 256; i++) {
        for (int j = 0; j < huffdata[i]; j++) {
            huff_add_ref(&msg_huffman.compressor, (byte) i); // Do update
            huff_add_ref(&msg_huffman.decompressor, (byte) i); // Do update
        }
    }
}
