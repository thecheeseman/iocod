...
// so the exe can com_memcpy these addresses over its own function redirects
...
{
    if (addr)
        com_memcpy(&_scr_get_bool, &addr, sizeof(intptr_t) * 102);
...
void scr_parse_gametype_list(void)
{
    ...
    com_memset(g_scr_data.gametypes, 0, sizeof(g_scr_data.gametypes));
    ...
    for (i = 0; i < nfiles; i++) {
        ...
        if (flen < 1 || flen > MAX_DESCRIPTION_LENGTH) {
            ...
            com_memset(description, 0, MAX_DESCRIPTION_LENGTH);
